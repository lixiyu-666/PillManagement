...
import { getCheckItems, createCheckRecord, updateCheckRecord, uploadImage, getCheckRecords } from '@/api'
...
const loadRecord = async () => {
  ...
  try {
    ...
    const record = records.find(r => r.id === Number(recordId.value))
    
    if (record) {
      form.value.checkItemId = record.checkItemId
      form.value.itemName = record.itemName
      form.value.checkDate = record.checkDate
      form.value.value = record.value.toString()
      form.value.unit = record.unit || ''
      form.value.reportImage = record.reportImage || ''
      
      if (record.reportImage) {
        fileList.value = [{
          url: record.reportImage,
          status: 'done'
        }]
      }
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateCheckRecord(recordId.value, data)
      showToast('修改成功')
    } else {
